...
# input file should have the following headers: Day, Start Time, End Time, Position
raise NotImplementedError('TODO: add path to spreadsheet')
FILENAME = ...
...
def fill_time(times, position):
    ...
    for i in range(2):
        tab(1)
        down(position) # TODO change this for multiple pay positions
        time.sleep(0.25)
...
for i in range(num_entries):
    ...
    position = 1 if math.isnan(row['Position']) else int(row['Position'])
    fill_time(times, position)
